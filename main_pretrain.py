...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        ######### insteatd of calling train_one_epoch method
        model.train(True)
        ...
        for data_iter_step, (samples, _) in enumerate(metric_logger.log_every(data_loader_train, print_freq, header)):
            ...
            with torch.cuda.amp.autocast():
                #latent, mask, ids_restore = model.module.forward_encoder(samples, mask_ratio=0.75)
                #pred = model.module.forward_decoder(latent, ids_restore)  # [N, L, p*p*3]
                #loss = model.module.forward_loss(samples, pred, mask)
                pred, imgs_, mask = model(samples, mask_ratio=args.mask_ratio)
            loss = model.module.forward_loss(imgs_, pred, mask)
            
            #loss_value = criterion(loss, samples)

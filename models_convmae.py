...
from load_vae import VAELoader
...
#        x_masked = torch.gather(x, dim=1, index=ids_keep.unsqueeze(-1).repeat(1, 1, D))
    ...
    def forward_loss(self, imgs, pred, mask): # img : GT -> VAE(x)
        """
        imgs: [N, 3, H, W]
        pred: [N, L, p*p*3]
    ...
    def forward(self, imgs, mask_ratio=0.75):
        ...
        # imgs_ = VAE.forward(imgs)
        print('hello: ', imgs.type(), next(self.vae.model.parameters()).type())
        print('hello: ', imgs.device, next(self.vae.model.parameters()).device)
        print(imgs.shape)
        imgs_ = self.vae.test(imgs)
        return pred, imgs_, mask
        #loss = self.forward_loss(imgs_, pred, mask)
